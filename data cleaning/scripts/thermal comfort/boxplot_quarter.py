...
def process_zone_data(input_file='rawdata.csv'):
    ...
    1. Filter for last quarter data (March, April, May)
    2. Calculate hourly averages for each zone
    3. Save separate CSV files for each zone in boxplot/quarter folder
    """
    # Create boxplot/quarter directory if it doesn't exist
    output_dir = os.path.join('boxplot', 'quarter')
    ...
    # Get unique zones
    zones = quarter_df['zone_id'].unique()
    print(f"\nFound {len(zones)} zones with last quarter data")
    ...
    for zone in zones:
        ...
        output_file = os.path.join(output_dir, f'lastquarter_zone{zone}.csv')
